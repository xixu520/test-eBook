...
| 维度 | 现状 | 问题 |
|------|------|------|
| 数据模型 | `StandardFile` 包含 `publisher`、`implementation_date`、`implementation_status`、`verify_status`、`number`、`version`、`year` 等硬编码字段 | 新增属性需改模型+迁移+前后端 |
| 表单系统 | `Form` / `FormField` 已建立，但仅用于分类绑定模板，未与文档数据关联 | 表单定义存在但无文档属性值存储 |
| 前端展示 | 首页 / 文档管理 / 编辑弹窗均硬编码属性列 | 无法自适应动态属性 |
